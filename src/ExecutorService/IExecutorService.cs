...
public interface IExecutorService : IExecutor, IDisposable, IAsyncDisposable
{
    ...
    /// <returns>A task that completes with the result, faults with the thrown exception, or is canceled by <see cref="ShutdownNow"/>.</returns>
    ...
    /// </summary>
    /// <param name="task">The task to submit.</param>
    /// <returns>A task that completes when the work finishes, faults with the thrown exception, or is canceled by <see cref="ShutdownNow"/>.</returns>
    /// <exception cref="RejectedExecutionException">The task cannot be scheduled for execution.</exception>
    /// <exception cref="ArgumentNullException"><paramref name="task"/> is <see langword="null"/>.</exception>
